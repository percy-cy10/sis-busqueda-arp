...
const columns = [
  ...
  // },
  {
    ...
    label: "Favorecidos",
    align: "center",
    field: (row) => row.favorecidos.map(f => f.nombre_completo).join(', '), // Mostrar nombres
    sortable: false,
  },
  {
    name: "otorgantes",
    label: "Otorgantes",
    align: "center",
    field: (row) => row.otorgantes.map(o => o.nombre_completo).join(', '), // Mostrar nombres
    sortable: false,
  },
...
// Función para eliminar una escritura
async function eliminar(id) {
  $q.dialog({
    ...
    await EscrituraService.delete(id);
    tableRef.value.requestServerInteraction();
    $q.notify({
      type: "positive",
