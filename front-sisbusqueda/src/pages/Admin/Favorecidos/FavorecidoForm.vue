...
// Inicialización condicional del formulario
let form;
if (props.edit && props.id) {
  form = useForm("put", `/api/favorecidos/${props.id}`, {
    tipo: "natural",
    nombre: "",
    apellido_paterno: "",
    apellido_materno: "",
  ...
  form = useForm("post", "/api/favorecidos", {
    tipo: "natural",
    nombre: "",
    apellido_paterno: "",
    apellido_materno: "",
...
const loadData = async (id) => {
  try {
    const data = await FavorecidoService.get(id);
    form.setData(data);
  } catch (error) {
