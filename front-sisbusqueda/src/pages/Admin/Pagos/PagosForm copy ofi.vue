          ...
          :rules="[
            ...
            (val) =>
              form.tipo_documento === 'RUC'
                ? (val && val.length === 11) || 'El RUC debe tener 11 dígitos'
                : true,
          ]"
...
onMounted(async () => {
  ...
  solicitudesOriginal.value = (
    (await SolicitudService.getData({ params: { rowsPerPage: -1 } })).data || []
  ).map((s) => ({
    ...s,
    id: Number(s.id),
    label: `N° = ${s.id} - Solicitante: ${
      s.solicitante?.nombre_completo || "-"
    }`,
  }));
...
async function autocompletarSolicitante() {
  if (
    form.value.tipo_documento === "DNI" &&
    form.value.num_documento.length === 8
  ) {
  ...
  if (
    form.value.tipo_documento === "RUC" &&
    form.value.num_documento.length === 11
  ) {
...
function actualizarTotal() {
  form.value.total = form.value.tupas.reduce(
    (sum, t) => sum + Number(t.Subtotal),
    0
  );
...
async function submit() {
  ...
  try {
    ...
    if (
      typeof form.value.tipo_documento === "object" &&
