      ...
      <q-form @submit.prevent="submit">
        ...
        <div class="col-12 q-px-lg q-mx-lg q-py-md">
          <q-option-group
            ...
            :options="[
              {
                label: `Copia Simple (${formatNumberToSoles(precios[0])})`,
                value: 0,
              },
              {
                label: `Copia certificada (${formatNumberToSoles(precios[1])})`,
                value: 1,
              },
              {
                label: `Testimonio (${formatNumberToSoles(precios[2])})`,
                value: 2,
              },
            ]"
          />
          <div class="col-12 row justify-end">
            <div class="col-7">
              <div class="q-pa-sm text-weight-bold text-subtitle1 text-right">
                Costo de Verificación :
              </div>
              <div class="q-pa-sm row items-center justify-end">
                <span class="q-pr-md text-weight-bold text-subtitle1"
                  >Cantidad de Folios :</span
                >
                <q-btn
                  round
                  color="primary"
                  ...
                  size="xs"
                  @click="cantiFolio > 1 ? (cantiFolio = cantiFolio - 1) : null"
                  :disable="true"
                />
                ...
                <q-btn
                  ...
                  icon="add"
                  size="xs"
                  @click="cantiFolio < 8 ? (cantiFolio = cantiFolio + 1) : null"
                  :disable="true"
                />
              </div>
              <div class="q-pa-sm row items-center justify-end">
                <span class="q-pr-md text-weight-bold text-subtitle1"
                  >Cantidad de copias :</span
                >
                <q-btn
                  round
                  color="primary"
                  icon="horizontal_rule"
                  size="xs"
                  @click="cantiCopia > 1 ? (cantiCopia = cantiCopia - 1) : null"
                />
                ...
                <q-btn
                  ...
                  icon="add"
                  size="xs"
                  @click="cantiCopia < 8 ? (cantiCopia = cantiCopia + 1) : null"
                />
              </div>
              <div class="q-pa-sm text-weight-bold text-subtitle1 text-right">
                Total :
            ...
            <div class="col-5">
              <div class="q-pa-sm text-subtitle1">
                {{ formatNumberToSoles(form.pago_verificacion) }}
              </div>
              <div class="q-pa-sm text-subtitle1">
                {{ formatNumberToSoles((form.pago_folio = Pagofolio)) }}
              </div>
              <div class="q-pa-sm text-subtitle1 lineinferior">
                {{ formatNumberToSoles((form.pago_fotocopia = Pagocopia)) }}
              </div>
              <div class="q-pa-sm text-subtitle1 text-green-13">
                {{ formatNumberToSoles(total) }}
              </div>
              <div class="q-pa-sm text-subtitle1">
                {{ formatNumberToSoles(montoEntrega) }}
              </div>
            </div>
            <div
              v-if="montoEntrega && parseInt(montoEntrega) !== 0"
              class="col-7 q-pa-sm text-weight-bold text-subtitle1 text-right"
            >
              Monto de Cambio (vuelto) :
            </div>
            <div
              v-if="montoEntrega && parseInt(montoEntrega) !== 0"
              class="col-5 q-pa-sm text-yellow-9 text-subtitle1 linesuper"
            >
        ...
        <div v-if="verPDF" class="col-12">
          <GenerarPDFSolicitud
            ...
            :datosVerificacion="D_verificacion"
            :ver="true"
            height="450px"
          />
        </div>
...
const Pagofolio = computed(() => {
  return (
    redondearConDecimales(precios.value[tipoCopia.value]) * cantiFolio.value
  );
});
const Pagocopia = computed(() => {
  return (
    cantiFolio.value *
    cantiCopia.value *
    redondearConDecimales(CostofotoCopia.value)
  );
});
...
async function getPrecios() {
  for (const key of cod_precios) {
    precios.value.push(
      await TupaService.getData({ params: { precio_sub_code: key } })
    );
  }
  CostofotoCopia.value = await TupaService.getData({
    params: { precio_sub_code: "0501" },
  });
}
