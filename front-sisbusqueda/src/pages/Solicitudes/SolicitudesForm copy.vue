      ...
      try {
        ...
        if (!solicitudId) {
          try {
            ...
            if (Array.isArray(listaSolicitudes.data)) {
              const ordenadas = listaSolicitudes.data.sort(
                (a, b) => b.id - a.id
              );
        ...
        if (!solicitudId) {
          $q.notify({
            type: "negative",
            message:
              "No se pudo obtener el ID de la solicitud. El pago no se guardará.",
          });
          return;
        }

        // Buscar el TUPA de búsqueda de documentos (sub_code 0101)
        const tupaBusquedaResp = await TupaService.getData({
          params: { sub_code: "0101" },
        });
        ...
        const pagoPayload = {
          ...
          user_id: userId,
          estado: 0,
          // monto_pagado: montoPagado, // ← AÑADIR ESTO
          // vuelto: vuelto, // ← AÑADIR ESTO
          tupas: [
            {
        ...
        if (pagoId) {
          await PagoService.toggleEstado(pagoId, {
...
async function enviarSinPago() {
  try {
    ...
    $q.notify({
      type: "positive",
      message: "Solicitud enviada y pago generado sin boleta.",
    });
    emit("save", "solicitud");
  } catch (error) {
    if (error.response && error.response.data && error.response.data.errors) {
