...
<template>
  <q-select
    v-if="cod_departamento === null || show_departamento"
    use-input
    hide-selected
    fill-input
    input-debounce="0"
    ...
    :rules="
      requerido
        ? [(val) => val !== null || 'Por favor seleccione departamento']
        : []
    "
    :options="optionsDepartamentos"
    option-label="nombre"
    option-value="cod_dep"
    @filter="filterDepartamentos"
    :loading="cargarComponent || loading"
    :disable="cargarComponent || props.cod_departamento !== null"
  >
    <template v-slot:label>
      Departamento
      <span v-if="requerido" class="text-red-7 text-weight-bold"
        >(*)</span
      ></template
    >
  ...
  <q-select
    v-if="cod_provincia === null || show_provincia"
    use-input
    hide-selected
    fill-input
    input-debounce="0"
    ...
    :rules="
      requerido
        ? [(val) => val !== null || 'Por favor seleccione provincia']
        : []
    "
    :options="optionsProvincias"
    option-label="nombre"
    option-value="cod_prov"
    :readonly="model_depa === null"
    @filter="filterProvincias"
    :loading="cargarComponent || loading"
    :disable="cargarComponent || props.cod_provincia !== null"
  >
    <template v-slot:label>
      Provincia
      <span v-if="requerido" class="text-red-7 text-weight-bold"
  ...
  <q-select
    ...
    :rules="
      requerido
        ? [(val) => val !== null || 'Por favor seleccione distrito']
        : []
    "
    :options="optionsDistritos"
    option-label="nombre"
    option-value="cod_dist"
    :readonly="model_prov === null"
    @filter="filterDistritos"
    :loading="cargarComponent || loading"
    :disable="cargarComponent"
  >
    <template v-slot:label>
      Distrito
      <span v-if="requerido" class="text-red-7 text-weight-bold"
  ...
  emit(
    ...
    _model
      ? typeof _model === "object"
        ? _model.cod_dep + _model.cod_prov + _model.cod_dist
        : _model
      : null
  );
}
...
onBeforeMount(async () => {
  ...
  [stringDepartamentos, allProvincias, allDistritos] =
    await UbigeosService.getAllUbigeo();
  [auxi_dep, auxi_pro, auxi_dis] = separarCadena(props.modelValue);
  if (auxi_dep) {
    getProvincias(auxi_dep);
    CargarModelDep(auxi_dep);
  }
  if (auxi_pro) {
    getDistritos(auxi_dep, auxi_pro);
    CargarModelPro(auxi_pro);
  }
...
function getDistritos(cod_dep, cod_prov) {
  if (cod_dep && cod_prov)
    stringDistritos = allDistritos.filter(
      (v) => v.cod_dep === cod_dep && v.cod_prov === cod_prov
    );
}
...
watch(
  () => model_dist.value,
  (newval, oldval) => {
    if (newval && auxi_dis !== newval.cod_dist) auxi_dis = newval.cod_dist;
    emitir(newval);
  }
);
watch(
...
function CargarModelDep(_dep) {
  model_depa.value = _dep
    ? stringDepartamentos.find((v) => v.cod_dep === _dep)
    : _dep;
}
function CargarModelPro(_pro) {
  model_prov.value = _pro
    ? stringProvincias.find((v) => v.cod_prov === _pro)
    : _pro;
}
function CargarModelDis(_dis) {
  model_dist.value = _dis
    ? stringDistritos.find((v) => v.cod_dist === _dis)
    : _dis;
}
/*******  para los filtros *********************************************************** */
