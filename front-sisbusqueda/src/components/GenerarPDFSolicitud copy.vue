...
function generarContenidoMinisterioPublico(doc, datos, maxWidth, montoBusqueda, boletaPago, estadoPago) {
  ...
  doc.text("N° de Caso", 25, yPos);
  doc.text(": " + (datos.numero_caso_mp || ''), 60, yPos);
  yPos += 8;

  doc.text("N° de Paquete", 25, yPos);
  doc.text(": " + (datos.numero_paquete_mp || ''), 60, yPos);
  yPos += 10;
...
function generarContenidoGenerico(doc, datos, maxWidth, montoBusqueda, boletaPago, estadoPago) {
  ...
  if (datos.tipo_tramite === 'procesos') {
    ...
    if (datos.notario_proceso) {
      ...
      doc.text(": " + limitarLineas(doc, datos.notario_proceso, 120, 2), 60, yPos, { align: "justify", maxWidth: 130 });
      yPos += 10;
    }
  }
...
async function generarPDF(datos) {
  ...
  const parrafo2 = `        Que amparado en los Dispositivos Legales Vigentes, Solicito se me expida el documento de acuerdo a los siguientes detalles: `;
  ...
  switch (datos.tipo_tramite) {
    ...
    default:
      ...
      doc.text("TRÁMITE SOLICITADO:", 20, 90);
      doc.line(20, 92, 70, 92);
      doc.text("Tipo de Trámite: " + (datos.tipo_tramite || ''), 25, 100);
      yPosFinal = 110;
  }
...
async function VerificaDatos(){
  ...
  if(props.datosSolicitudRow){
    ...
    datosEnPDF = {
      ...
      notario_proceso: props.datosSolicitudRow.notario_proceso,
