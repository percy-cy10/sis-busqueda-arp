  ...
  function generarContenidoPartidas(doc, datos, maxWidth, montoBusqueda, boletaPago, estadoPago) {
    ...
    if (datos.tipo_partida === 'defuncion' && datos.nombre_fallecido) {
      doc.text("Nombre Fallecido", 25, yPos);
  ...
  async function generarPDF(datos) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setFontSize(9);
      doc.setTextColor(0, 0, 0);
      doc.text("ARCHIVO REGIONAL - ", 20, pageHeight - marginBottom + 4);
      ...
      const codigoSolicitud = "S-" + datos.solicitud_code.toString().padStart(5, '0');
      const urlSeguimiento = `www.archivoregional.gob.pe/seguimiento?codigo=${codigoSolicitud}`;
      doc.setFontSize(9);
      doc.text(`CONSULTA EN: ${urlSeguimiento}`, 56, pageHeight - marginBottom + 4);
  ...
  async function VerificaDatos(){
    ...
    if(props.datosSolicitudRow){
      ...
      datosEnPDF = {
        ...
        solicitud_code: props.datosSolicitudRow.solicitud_code,
