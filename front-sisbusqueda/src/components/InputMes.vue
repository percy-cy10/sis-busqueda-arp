<template>
  <q-input
    ...
    :readonly="readonly"
    lazy-rules
    @click="readonly ? (modelpopup = false) : (modelpopup = true)"
    :rules="
      requerido
        ? [(val) => (val && val !== '' && inMes(val)) || 'Ingrese un Mes']
        : [
            (val) =>
              val === null ||
              val === '' ||
  ...
  >
    <!-- <q-input
      :model-value="modelMes"
      :label="label"
      bottom-slots
      mask="Aaaaaaaaa"
      :readonly="readonly"
      lazy-rules
      @update:model-value="val => modelMes = val"
      @click="readonly ? modelpopup=false : modelpopup=true"
      :rules="requerido
        ? [val => ( val && val !== '' && inMes(val)) || 'Ingrese un Mes']
        : [val => val===null || val === '' || inMes(val) || 'Ingrese un Mes válido']"> -->
        ...
        <div class="col-12 row q-pa-sm" style="max-width: 280px">
          ...
          >
            <span
              ...
              "
              class="classMes"
              @click="PermitirMes(index + 1) ? asignar(mes) : null"
            >
              {{ mes }}
            </span>
          </div>
...
const props = defineProps({
  ...
  RangoFechas: {
    default: ["01/01/1901", convertDate(new Date(), "dd/MM/yyyy")],
  },
...
/***  validación de años **************************** */
const [anio_ini, anio_fin] = props.RangoFechas
  ? props.RangoFechas.map((fecha) => parseInt(fecha.split("/")[2]))
  : [null, null];
/*** validacion de mees***************************************** */
function inMes(val) {
  return listMes.includes(val);
}
const [mes_ini, mes_fin] =
  anio_ini && anio_fin
    ? props.RangoFechas.map((fecha) => parseInt(fecha.split("/")[1]))
    : [null, null];
