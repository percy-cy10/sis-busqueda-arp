...
class EscrituraController extends Controller
{
    ...
    public function buscar(Request $request)
    {
        ...
        // 🔎 Filtro por nombres (otorgantes o favorecidos)
        if ($request->filled('nombres') && strlen($request->nombres) >= 2) {
            $valor = $request->nombres;
            $query->where(function($q) use ($valor) {
                $q->whereHas('otorgantes', fn($q2) =>
                    $q2->where('nombre_completo', 'LIKE', "%$valor%"))
                ->orWhereHas('favorecidos', fn($q2) =>
                    $q2->where('nombre_completo', 'LIKE', "%$valor%"));
            });
        }
        ...
        // Otros filtros
        foreach (['otorgante' => 'otorgantes', 'favorecido' => 'favorecidos'] as $campo => $relacion) {
            if ($request->filled($campo)) {
                $valor = $request->$campo;
                $query->whereHas($relacion, fn($q) =>
                    $q->where('nombre_completo', 'LIKE', "%$valor%"));
            }
        }

        foreach (['notario', 'bien', 'cod_escritura'] as $campo) {
            if ($request->filled($campo)) {
                $valor = $request->$campo;
        ...
        $escrituras = $query->paginate($perPage, ['*'], 'page', $page);
